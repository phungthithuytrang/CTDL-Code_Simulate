...
int main()
{
	outfile1.open("E:\\CTDL\\CTDLProject\\Test ct\\Simulate\\timecompareBlock.txt", ios::out | ios::trunc);
	outfile2.open("E:\\CTDL\\CTDLProject\\Test ct\\Simulate\\kqBlock.txt", ios::out | ios::trunc);
	...
	for (int i = 0; i < 15; i++)
	{
		x1 = x + std::to_string(i + 1) + ".txt";
		x2 = x + std::to_string(i + 2) + ".txt";
		m = Getsize(x1);
		n = Getsize(x2);
